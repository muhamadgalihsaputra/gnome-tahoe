#!/usr/bin/env bash
# Laptop Mode - NO LAG VERSION
# Portable setup: enable fingerprint/touchpad, responsive performance
...
print_header() {
    clear
    echo -e "${CYAN}╔════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC}  ${BOLD}LAPTOP MODE${NC} - Portable Setup        ${CYAN}║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════════╝${NC}"
    echo ""
}
...
log "========== LAPTOP MODE START =========="

# Get sudo early
echo -e "${CYAN}Applying laptop mode (portable, responsive)...${NC}"
...
# Platform: Balanced (smooth + cool, NOT low-power!)
...
# Fingerprint
FP_STATUS=$(systemctl is-active fprintd.service 2>&1 | head -1 | tr -d ' \n\r')
if [[ "$FP_STATUS" == "active" ]]; then
    echo -e "  ${GREEN}✓${NC} Fingerprint: ${GREEN}ENABLED${NC}"
    log "Fingerprint: enabled ✓"
else
...
if [[ "$TP_STATUS" == "'enabled'" ]]; then
    echo -e "  ${GREEN}✓${NC} Touchpad: ${GREEN}ENABLED${NC}"
...
if [[ -n "$WIFI_DEV" ]]; then
    ...
    if [[ "$WIFI_PS" == "off" ]]; then
        echo -e "  ${GREEN}✓${NC} WiFi Power Save: ${GREEN}OFF${NC} (no delay!)"
...
echo -e "  ${GREEN}✓${NC} USB Autosuspend: ${GREEN}OFF${NC} ($USB_ON/$USB_COUNT devices, mouse ready!)"
...
log "Laptop mode complete"

# Summary
echo ""
echo -e "${GREEN}╔════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║${NC}  ${BOLD}Laptop Mode Applied${NC}                    ${GREEN}║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${CYAN}Configuration:${NC}"
echo -e "  Input: ${GREEN}Touchpad + Mouse${NC} (fingerprint enabled)"
echo -e "  CPU: ${GREEN}100%${NC} (no limit, full power!)"
echo -e "  Platform: ${GREEN}$PLATFORM${NC} (smooth + cool)"
echo -e "  WiFi: ${GREEN}Responsive${NC} (power save off, no delay!)"
echo -e "  USB: ${GREEN}Mouse ready${NC} (autosuspend off)"
echo -e "  Battery: ${BAT_START}-${BAT_STOP}% (GNOME controls)"
echo ""
echo -e "${GRAY}Note: Battery ~15-40min less than ultra-saver mode${NC}"
echo -e "${GRAY}      but NO LAG, smooth & responsive!${NC}"
echo ""
echo -e "${GRAY}Switch to desktop mode: desktop-mode${NC}"
echo ""
